...
{
    public static class GetCategoryArticlesQuery
    {
        public record Query(Guid CategoryId) : IRequest<IEnumerable<Article>>;
        public class Handler : IRequestHandler<Query, IEnumerable<Article>>
        {
            ...
            {
                return await repository.GetByCategoryAsync(request.CategoryId, cancellationToken);
            }
        }
    }
}
