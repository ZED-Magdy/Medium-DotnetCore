...
{
    public static class GetBlogArticlesQuery
    {
        public record Query(Guid Id) : IRequest<IEnumerable<Article>>;
        public class Handler : IRequestHandler<Query, IEnumerable<Article>>
        {
            ...
            {
                return await repository.GetByBlogAsync(request.Id, cancellationToken);
            }
        }
    }
}
